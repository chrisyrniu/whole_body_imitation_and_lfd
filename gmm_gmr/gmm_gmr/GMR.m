...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:nbStates
  Pxi(:,i) = Priors(i).*gaussPDF(x, Mu(in,i), Sigma(in,in,i)); %%N*1
end
beta = Pxi./repmat(sum(Pxi,2)+realmin,1,nbStates); %%N*K
%% Compute expected means y, given input x
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j=1:nbStates
  y_tmp(:,:,j) = repmat(Mu(out,j),1,nbData) + Sigma(out,in,j)*inv(Sigma(in,in,j)) * (x-repmat(Mu(in,j),1,nbData));%%Q*N+Q*P*P*P*(P*N-P*N)=Q*N
end
beta_tmp = reshape(beta,[1 size(beta)]);%%1*N*K
y_tmp2 = repmat(beta_tmp,[length(out) 1 1]) .* y_tmp;%%Q*N*K
y = sum(y_tmp2,3);%Q*N
%% Compute expected covariance matrices Sigma_y, given input x
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j=1:nbStates
  Sigma_y_tmp(:,:,1,j) = Sigma(out,out,j) - (Sigma(out,in,j)*inv(Sigma(in,in,j))*Sigma(in,out,j)); %% Q*Q-Q*P*P*P*P*Q
end 
beta_tmp = reshape(beta,[1 1 size(beta)]);%%1*1*N*K
Sigma_y_tmp2 = repmat(beta_tmp.*beta_tmp, [length(out) length(out) 1 1]) .* repmat(Sigma_y_tmp,[1 1 nbData 1]);%%Q*Q*N*K
Sigma_y = sum(Sigma_y_tmp2,4);%Q*Q*N
