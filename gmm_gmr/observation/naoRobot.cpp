...
naoRobot::naoRobot(boost::shared_ptr<AL::ALBroker> broker,const std::string& name): AL::ALModule(broker, name)
{
	...
	functionName("clock", getName(), "clock");
	BIND_METHOD(naoRobot::clock1);
...
void naoRobot::startImitation(_kinectData kinectData)
{
	...
	clock1();
	if(FrameChange!=naoJointAngles[NAO_RIGHT_SHOULDER_PITCH])observation();
	FrameChange=naoJointAngles[NAO_RIGHT_SHOULDER_PITCH];
}
void naoRobot::clock1()
{
	...
	{
		clockInitial=clock();
		outfile.open("2chenzhounan3.txt");
	}
...
void naoRobot::observation()
{
	outfile<<naoJointAngles[NAO_RIGHT_SHOULDER_PITCH]<<","<<naoJointAngles[NAO_RIGHT_SHOULDER_ROLL]
	<<","<<naoJointAngles[NAO_RIGHT_ELBOW_YAW]<<","<<naoJointAngles[NAO_RIGHT_ELBOW_ROLL]<<","<<time_total<<endl;
	cout<<naoJointAngles[NAO_RIGHT_SHOULDER_PITCH]<<","<<naoJointAngles[NAO_RIGHT_SHOULDER_ROLL]
	<<","<<naoJointAngles[NAO_RIGHT_ELBOW_YAW]<<","<<naoJointAngles[NAO_RIGHT_ELBOW_ROLL]<<","<<time_total<<endl;
}