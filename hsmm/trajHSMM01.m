...
%% Parameters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
model.nbStates = 8; %Number of components in the GMM
model.nbVarPos = 2; %Dimension of position data (here: x1)
...
% 			oTmp = prod(c(1:t) .* gaussPDF(s(5).Data(:,1:t), model.Mu(:,i), model.Sigma(:,:,i))); %Observation probability for standard HSMM
			h(i,t) = model.StatesPriors(i) * model.Pd(i,t) * oTmp;
		end
		for d=1:min(t-1,nbD)
			oTmp = 1; %Observation probability for generative purpose
% 			oTmp = prod(c(t-d+1:t) .* gaussPDF(s(5).Data(:,t-d+1:t), model.Mu(:,i), model.Sigma(:,:,i))); %Observation probability for standard HSMM	
...
for m=1:model.nbVarPos
	limAxes = [1, nbData, min(Data(m,:))-2E1, max(Data(m,:))+2E1];
...
for m=1:model.nbVarPos
	...
	msh = [1:nbData, nbData:-1:1; r(1).Data(m,:)-squeeze(r(1).expSigma(m,m,:).^.125)'*1, fliplr(r(1).Data(m,:)+squeeze(r(1).expSigma(m,m,:).^.125)'*1)];
